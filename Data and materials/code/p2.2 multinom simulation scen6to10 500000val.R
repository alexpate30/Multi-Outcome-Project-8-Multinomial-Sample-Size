...
scenario6.res<-(foreach(input=c(100,250,500,1000,177,189,196,186,195), .combine=list, .multicombine=TRUE)
...
warnings()
Sys.time()
save.image("R_out/multinom simulation scen6to10 500000val.RData")
...
cl <- makeCluster(9)
registerDoParallel(9)
scenario7.res<-(foreach(input=c(100,250,500,1000,188,237,219,190,192), .combine=list, .multicombine=TRUE)
                %dopar%{run.multinom.sim(0, 1, -0.5, -0.25, 0.5, 0.75,
                                         -1, 0.75, -1, -0.5, -0.75, 0.25, 
...
cl <- makeCluster(9)
registerDoParallel(9)
scenario8.res<-(foreach(input=c(100,250,500,1000,185,238,197,185,186), .combine=list, .multicombine=TRUE)
...
cl <- makeCluster(9)
registerDoParallel(9)
scenario9.res<-(foreach(input=c(250,500,1000,208,338,289,252,304), .combine=list, .multicombine=TRUE)
...
cl <- makeCluster(9)
registerDoParallel(9)
scenario10.res<-(foreach(input=c(250,500,1000,233,526,455,354,554), .combine=list, .multicombine=TRUE)
